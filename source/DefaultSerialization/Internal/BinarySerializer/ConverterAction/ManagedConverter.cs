...
namespace DefaultSerialization.Internal.BinarySerializer.ConverterAction
{
    internal static class ManagedConverter
    {
        public static (WriteAction<T>, ReadAction<T>) GetActions<T>()
        {
            ...
            DynamicMethod writeMethod = new("Write", typeof(void), [typeof(StreamWriterWrapper).MakeByRefType(), type.MakeByRefType()], typeof(ManagedConverter), true);
            ILGenerator writeGenerator = writeMethod.GetILGenerator();

            DynamicMethod readMethod = new("Read", type, [typeof(StreamReaderWrapper).MakeByRefType()], typeof(ManagedConverter), true);
